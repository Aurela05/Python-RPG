...
class Upgrade:
    ...
    def input(self):
        ...
        moo = pygame.mouse.get_pressed(num_buttons=3)
        if self.can_move:
            if keys[pygame.K_d] and self.selection_index < self.attribute_num - 1:
...
class Item:
    ...
    def trigger(self,player):
        ...
        if player.exp >= player.upgrade_cost[upgrade_attribute] and player.stats[upgrade_attribute] < player.max_stats[upgrade_attribute]:
            ...
            print(player.e_recovery_speed)
            
            if self.index == 3:
                player.flame_strength += 1
                player.heal_strength += 0.25
            
            elif self.index == 1:
                player.e_recovery_speed += 0.004
